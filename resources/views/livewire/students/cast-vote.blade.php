...
new #[Layout('layouts.app')] #[Title('Digital Ballot')] class extends Component {
    ...
    public function mount()
    {
        ...
        $this->currentPositionIndex = session('election_position_index', 0);
        $this->selections = session('election_selections', []);
        $this->currentStep = session('election_current_step', 1);
        ...
        if ($this->isVotingOpen) {
            ...
            foreach ($this->electionData as $position) {
                $vote = $existingVotes->firstWhere('position_id', $position->id);

                if ($vote) {
                    $this->selections[$position->id] = $vote->candidate_id;
                } elseif (!isset($this->selections[$position->id])) {
    ...
    public function nextPosition()
    {
        ...
        if ($this->isLastPosition) {
            ...
            session(['election_position_index' => $this->currentPositionIndex]);
    ...
    public function submitVote()
    {
        ...
        if (!isset($result['error'])) {
            session()->forget(['election_selections', 'election_position_index', 'election_current_step']);
        }
...
<div>
    <div
        ...
        <main class="main-content">
            ...
            @if ($this->hasVoted)
                <div class="fade-in d-flex flex-column align-items-center justify-content-center"
                    ...
                    <div class="glass-card p-5 text-center shadow-sm border-0 bg-white"
                        ...
                        <a href="/students/dashboard" wire:navigate class="btn btn-glow w-100 py-2">Return to
                            Dashboard</a>
                    </div>
                </div>
                ...
                <div class="d-flex flex-column align-items-center justify-content-center" style="min-height: 65vh;">
                    <div class="text-center" style="max-width: 550px; border-radius: 25px;">
                        ...
                        <p class="text-secondary mx-auto mb-4" style="max-width: 500px;">
                            There is no active election cycle at this time. Please monitor the official announcements
                            from
                    ...
                    @foreach ([1 => 'Select', 2 => 'Review', 3 => 'Confirm'] as $num => $label)
                        <div
                            ...
                            <div class="step-circle"
                                style="width: 28px; height: 28px; font-size: 12px; line-height: 28px;">
                                {{ $num }}
                            </div>
                ...
                @if ($currentStep == 1)
                    ...
                    @if ($position)
                        <div class="fade-in px-2 py-2" wire:key="position-{{ $position->id }}">
                            ...
                            <div class="mt-8 flex justify-end pb-10 mb-3 md:mb-4">
                                <button wire:click="nextPosition" wire:loading.attr="disabled"
                                    wire:target="nextPosition"
                    ...
                    @else
                        <div class="text-center py-20">
                            <div
                                class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gray-50 mb-4">
                    ...
                    <div class="fade-in px-2 py-3">
                        ...
                        <div class="grid grid-cols-2 md:flex md:flex-wrap justify-center gap-3 md:gap-4 mb-12">
                            @forelse ($this->electionData as $position)
                                @php
                                    $selectedId = $selections[$position->id] ?? null;
                                    $candidate = $position->candidates->firstWhere('id', $selectedId);
                                @endphp
                        ...
                        <div class="mt-8 flex items-center justify-end gap-4 pb-10 mb-3 md:mb-4">
                            <button wire:click="$toggle('showNames')"
                                ...
                                <i class="bi {{ $showNames ? 'bi-eye-slash' : 'bi-eye' }} text-lg mt-1"></i>
                        ...
                        <div
                            ...
                            <div class="mb-6">
                                <div
                                    ...
                                    <i
                                        class="bi bi-shield-lock-fill text-[#10b981] text-[1.8rem] md:text-[2.5rem]"></i>
                                </div>
